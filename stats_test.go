...
// Regression: ValueCounts on a column literally named "count" used to
// collide with the result's "count" column and silently drop the labels.
func TestValueCountsOnColumnNamedCount(t *testing.T) {
	...
	result, err := df.ValueCounts("count")
	if err != nil {
		t.Fatal(err)
	}
	if _, cols := result.Shape(); cols != 2 {
